...
for p in 9000 9001 9002; do
  sudo systemctl enable --now dbridge-relay@"$p".service >/dev/null
  curl -fsS "http://127.0.0.1:$((10000+p))/healthz" >/dev/null || { echo "FAIL :$p"; exit 1; }
done
...
echo "systemd demo" > demo.txt

python3 dbridge.py --tcp --chain "$CHAIN" --secrets secrets.json --pad 512 --mtu 700 sendfile demo.txt
